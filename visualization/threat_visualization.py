...
lstm_model.load_state_dict(
    torch.load("models/lstm_model.pth", map_location=torch.device("cpu"))
)
...
track_history = {}
email_sent = set()
print("email_sent at start:", email_sent)
# -----------------------------
# Restricted Zone
# -----------------------------
ZONE_X1 = 900
        ...
        for track_id, box in zip(ids, coords):
            ...
            if len(points) >= 8:
                ...
                object_class = yolo_model.names[
                    int(boxes.cls.cpu().tolist()[ids.index(track_id)])
                ]

                score = calculate_threat_score(future_points, object_class, zone)
...
"""
                        ...
                        print("Already emailed:", track_id)
                    cv2.rectangle(
    ...
    cv2.imshow("AI Threat Surveillance System", resized)
